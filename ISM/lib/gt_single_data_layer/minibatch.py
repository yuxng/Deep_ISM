...
def get_minibatch(roidb, voxelizer):
    ...
    if cfg.TRAIN.VISUALIZE:
        _vis_minibatch(im_blob, im_depth_blob, depth_blob, label_blob, vertex_target_blob, voxelizer.num_classes)
...
def _process_label_image(label_image, class_colors, class_weights):
    ...
    label_index = np.zeros((height, width, 1), dtype=np.float32)
...
def _vis_minibatch(im_blob, im_normal_blob, depth_blob, label_blob, vertex_target_blob, num_classes):
    ...
    for i in xrange(im_blob.shape[0]):
        ...
        # show normal image
        im_normal = im_normal_blob[i, :, :, :].transpose((1, 2, 0)).copy()
        ...
        # show label
        label = label_blob[i, :, :, :].transpose((1, 2, 0))
